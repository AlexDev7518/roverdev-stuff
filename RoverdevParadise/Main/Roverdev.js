...
module.exports = Roverdev => {
    ...
    let guildId = "846548733914906664";
    let roleId = "1085506977477709904";
    ...
    Roverdev.on("presenceUpdate", async (oP, nP) => {
        ...
        if(nP){
            ...
            if(nP.activities.some(({ state }) => state?.includes(textInStats)) && !m.user.bot) {
                if(!m.roles.cache.has(roleId)) { // add role if user doesn't have it
                    m.roles.add(roleId, "Status Added, Role Added").catch(() => {});
                }
            } else {
                if(m.roles.cache.has(roleId) && !m.user.bot) { //removes role if user has it
                    m.roles.remove(roleId, `Status Removed, Role Removed`).catch(() => {});
                }
            }
        }
    })
    ...
    Roverdev.on("ready", async () => {
        ...
        for(const m of members_who_need){
            await m.roles.add(roleId, `Status Added, Role Added`).catch(() => {});
            await delay(350); //wait 0.35 secs
        }
        //remove the roles for those who needs it
        for(const m of members_who_remove){
            await m.roles.remove(roleId, `Status Removed, Role Removed`).catch(() => {});
            await delay(350); //wait 0.35 secs
        }
         }, 5000);
    })
