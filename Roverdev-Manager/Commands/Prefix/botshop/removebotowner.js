...
const {  RemoveBotOwner } = require("../../../Modules/Rover-BotShop/Functions")
   ...
   CommandRun: async (Roverdev, message, args, executor) => {
           ...
           if (args[0].includes("@")) {
                  ...
                  if (user.bot == true) {
                       ...
                       RemoveBotOwner(BotInfo.BotPath, BotInfo.Bot ,ExtraOwner).then(async (res) => {
                               if (res) {
                                        ...
                                        return msg.edit({ content: `Successfully Removed ${ExtraOwner} From <@${user.id}> Owners!` })
                               }
                       }).catch((error) => {
