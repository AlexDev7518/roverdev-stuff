  ...
  setTimeout(async () => {     
             if (data) {
                    ...
                    if (joins.Leaves.includes(member.user.id)) {
                         Channel1.send({ content: `<a:MemberLeftGuild:1080740946435637279> Goodbye ${member} From \`${member.guild.name}\`\n> Invited By: \`${user}\` - Uses: \`${joins.uses}\`\n> -  Invites:  \`${joins.Joins}\`  |  Fakes: \`${joins.Fakes}\`  | Leaves: \`${joins.Leaves.length}\`` })                  
               ...
               const res1 = await Roverdev.guilds.cache.get("1081700920993259550").fetchVanityData().catch(() => {});
          ...
          stuff.forEach(m => {
               try {
                     ...
                     return console.log(`Seems there is nothing for ${member.id} in ${m}`)
               }
