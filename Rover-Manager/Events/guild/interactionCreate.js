...
module.exports = async(client, interaction) => {
    ...
    if (interaction.isAutocomplete()) {
        if (interaction.commandName === 'botshop') {
           ...
           const choices = [];
                      if (Bots.length > 0) {
                                  Bots.forEach(bot => { choices.push( { BotName: client.users.cache.get(bot).username, BotID: bot } ) })
                      } else if (Bots.length < 1) {
                        choices.push( { BotName: "You have no bots", BotID: "invaild" } )
                }
                
           await interaction.respond(
            choices.map(choice => ({ name: choice.BotName, value: choice.BotID })),
        );
        }
   }
			...
			try {
                   ...
                   if (command.conf.Slash.cooldown) {
					...
					if (Timeout.has(interaction.user.id)) {
                        ...
                        if (now < expirationTime) {
                            let timeLeft = (expirationTime - now) / 1000; //get the lifetime
                ...
                setTimeout(() => {
                    Timeout.delete(`${interaction.user.id}${command.help.Slash.name}`)
                        ...
                        console.log(`${command.help.Slash.name} ran by: ${interaction.user.username}`)
