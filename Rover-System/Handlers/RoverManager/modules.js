...
module.exports = async (Rover) => {
  ...
  readdirSync("./Handlers/RoverManager/Modules").forEach(CommandDirectory => {
      const CommandsInFolders = readdirSync("./Handlers/RoverManager/Modules/" + `${CommandDirectory}/`).filter(f => f.endsWith(".js"))

      for (const Event of CommandsInFolders) {

          require(`../../Handlers/RoverManager/Modules/${CommandDirectory}/${Event}`)(Rover)
